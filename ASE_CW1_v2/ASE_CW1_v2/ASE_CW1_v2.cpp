...
void update_western_wall(const std::map<std::string, std::string> brick_names, std::list<std::string>& result)
{
    while (brick_names.find(result.front()) != brick_names.end()) {
        const std::map<std::string, std::string>::const_iterator found_at = brick_names.find(result.front());
        result.push_front(found_at->second);
    }
}

void update_eastern_wall(const std::map<std::string, std::string> brick_names, std::list<std::string>& result)
{
    while (brick_names.find(result.back()) != brick_names.end()) {
        const std::map<std::string, std::string>::const_iterator found_at = brick_names.find(result.back());
...
std::list<std::string> get_results(std::string path) {
    ...
    const std::map<std::string, std::string>::const_iterator start_point = brick_names.inorder.begin();
